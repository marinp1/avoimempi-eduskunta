...
function parseLausunto(
  ...
  collectTextFragments(lausunto.SisaltoKuvaus, summaryParts);
...
export default function createValiokunnanLausuntoSubMigrator(db: Database) {
  const insertReport = db.prepare(
    `INSERT INTO CommitteeReport (id, parliament_identifier, report_type_code, document_number, parliamentary_year, title, committee_name, recipient_committee, source_reference, draft_date, signature_date, language, edk_identifier, summary_text, general_reasoning_text, detailed_reasoning_text, decision_text, legislation_amendment_text, minority_opinion_text, resolution_text, source_path)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'fi', ?, ?, ?, ?, ?, ?, ?, ?, ?)
