...
function writeMigrationReport(
  ...
  const baseDir =
    ...
    join(process.cwd(), "data", "migration-reports", "VaskiData", "suullinen_kysymys");
...
export default function createSuullinenKysymysSubMigrator(db: Database) {
  ...
  const deleteSubjects = db.prepare("DELETE FROM OralQuestionSubject WHERE question_id = ?");
      ...
      try {
        ...
        if (data.subjects.length > 0) {
          ...
          for (const subject of data.subjects) {
            insertSubject.run(questionId, subject.subject_text, subject.yso_uri);
          }
        }
      } catch (error) {
