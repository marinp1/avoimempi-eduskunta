...
function writeMigrationReport(
  ...
  const baseDir =
    ...
    join(process.cwd(), "data", "migration-reports", "VaskiData", "lakialoite");
...
function parseParliamentIdentifier(eduskuntaTunnus: unknown): {
  ...
  const match = normalized.match(/^LA\s+(\d+)\/(\d+)\s*(?:vp)?$/i);
...
function parseLakialoite(
  ...
  const la = body.Lakialoite;
  if (!la || typeof la !== "object") {
    throw new Error(`Missing Lakialoite body (${context})`);
  }
...
export default function createLakialoiteSubMigrator(db: Database) {
  ...
  return {
    async migrateRow(row: VaskiEntry): Promise<void> {
      ...
      const sourcePath = row._source?.vaskiPath
        ...
        : `vaski-data/lakialoite/unknown#id=${id}`;

      const isLakialoite = !!body.Lakialoite;
      const isKasittelytiedot = !!body.KasittelytiedotValtiopaivaasia;

      if (!isLakialoite && !isKasittelytiedot) {
        writeMigrationReport(
          row,
          "unknown_body_type",
          `Body has neither Lakialoite nor KasittelytiedotValtiopaivaasia (${context})`,
        );
        return;
      }
      ...
      try {
        if (isLakialoite) {
          const data = parseLakialoite(row, body, context);
