...
function parseParliamentIdentifier(eduskuntaTunnus: unknown): {
  ...
  const match = normalized.match(/^TAA\s+(\d+)\/(\d+)\s*(?:vp)?$/i);
  ...
  return {
    initiativeTypeCode: "TAA",
    identifier: `TAA ${number}/${year} vp`,
    number,
    year,
  };
}
...
export default function createTalousarvioaloiteSubMigrator(db: Database) {
  ...
  return {
    async migrateRow(row: VaskiEntry): Promise<void> {
      ...
      const sourcePath = row._source?.vaskiPath
        ...
        : `vaski-data/talousarvioaloite/unknown#id=${id}`;
