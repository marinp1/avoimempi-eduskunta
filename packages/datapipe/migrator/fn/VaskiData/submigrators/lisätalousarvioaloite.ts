...
function parseParliamentIdentifier(eduskuntaTunnus: unknown): {
  ...
  const match = normalized.match(/^LTA\s+(\d+)\/(\d+)\s*(?:vp)?$/i);
  ...
  return {
    initiativeTypeCode: "LTA",
    identifier: `LTA ${number}/${year} vp`,
    number,
    year,
  };
}
...
export default function createLisatalousarvioaloiteSubMigrator(db: Database) {
  ...
  return {
    async migrateRow(row: VaskiEntry): Promise<void> {
      ...
      const sourcePath = row._source?.vaskiPath
        ...
        : `vaski-data/lisätalousarvioaloite/unknown#id=${id}`;
