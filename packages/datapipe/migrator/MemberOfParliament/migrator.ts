...
export default (db: Database) => (dataToImport: DataModel.Representative) => {
  const XmlDataFi = JSON.parse(dataToImport.XmlDataFi);

  if (process.env.DEBUG)
    console.log("Mapping", dataToImport.lastname, dataToImport.personId);

  type RepresentativeInsert = Omit<DatabaseTables.Representative, "birth_year">;
  ...
  const { absenceRows, joiningRows, leavingRows }: ParsedAbsence = mergeArrays(
  ...
  const trustPositionRows: DatabaseTables.TrustPosition[] = [
    ...
    ...mergeArrays(XmlDataFi.Henkilo.KunnallisetLuottamustehtavat?.Tehtava).map(
      (s) => ({ ...s, type: "municapility" as const }),
    ),
