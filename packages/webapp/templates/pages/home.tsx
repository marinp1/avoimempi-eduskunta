...
/** String version of HomeBody for use in non-JSX contexts (HomeReactive). */
function HomeBodyStr(data: HomeData, sessionCount?: number): string {
  ...
  return `<hr class="rule"><div class="wrap">${statRowStr(comp.totalMembers, govTotal, oppTotal, sessionCount)}</div><hr class="rule"><div class="home-main wrap">${leftColStr(comp, govParties, oppParties, visParties, govTotal, oppTotal, closeVotes)}<div>${speakersStr(speakers)}</div></div>`;
}
...
function statRowStr(
  ...
  return `<div class="stat-row"><div class="stat"><div class="stat__label">Kansanedustajat</div><div class="stat__value">${total}</div></div><div class="stat"><div class="stat__label">Hallitus</div><div class="stat__value hall" data-tl-hall>${gov}</div></div><div class="stat"><div class="stat__label">Oppositio</div><div class="stat__value opp" data-tl-opp>${opp}</div></div><div class="stat"><div class="stat__label">Istuntoja kaudella</div><div class="stat__value" data-tl-statval>${sessionCount ?? "—"}</div></div></div>`;
}

function leftColStr(
  comp: HomeData["composition"],
  govParties: HomeData["composition"]["parties"],
  oppParties: HomeData["composition"]["parties"],
  visParties: HomeData["composition"]["parties"],
  govTotal: number,
  oppTotal: number,
  closeVotes: HomeData["signals"]["closeVotes"],
): string {
  const totalMembers = comp.totalMembers;
  const blocBar =
    govParties
      .map(
        (p) =>
          `<span class="seg-hall" style="width:${pct(p.member_count, totalMembers)};background:${partyColor(p.party_display_code)}" title="${partyShortName(p.party_display_code)} ${p.member_count}"></span>`,
      )
      .join("") +
    oppParties
      .map(
        (p) =>
          `<span class="seg-opp" style="width:${pct(p.member_count, totalMembers)};background:${partyColor(p.party_display_code)}" title="${partyShortName(p.party_display_code)} ${p.member_count}"></span>`,
      )
      .join("");
  const partyRows = visParties
    .map((p) => {
      ...
      const widthPct = pct(p.member_count, totalMembers);
      return `<tr><td class="pt-dot"><span style="background:${color}"></span></td><td class="pt-name">${name} <small>${bloc}</small></td><td class="pt-bar"><div class="track"><div class="fill" style="width:${widthPct};background:${color}"></div></div></td><td class="pt-seats">${p.member_count}</td></tr>`;
    })
    .join("");

  const closeVoteRows =
    closeVotes.length > 0
      ? `<div class="psec" style="margin-top:32px"><div class="psec__h"><h2>Tiukimmat äänestykset</h2><small class="kicker">vaalikausi tähän mennessä</small></div><div class="dissent">${closeVotes
          .map((v) => {
            const passed = v.n_yes >= v.n_no;
            const voteTitle = esc(v.title || v.section_title);
            return `<div class="vote-row"><div class="vote-row__badge${passed ? " jaa" : ""}">${passed ? "JAA" : "EI"}</div><div><div class="vote-row__title">${voteTitle}</div><div class="vote-row__sub"><span class="date">${formatDate(v.start_time)}</span><span>${esc(v.section_title)}</span></div></div><div class="vote-row__result"><div class="r-line">${v.n_yes} JAA – ${v.n_no} EI</div><div class="r-line"><span class="${passed ? "pass" : "fail"}">${v.margin} ään. ero</span></div></div></div>`;
          })
          .join("")}</div></div>`
      : "";

  return `<div><span class="kicker">Poliittinen kokoonpano</span><div class="bloc-bar" style="margin-top:18px">${blocBar}</div><div class="bloc-legend"><span class="item"><span class="swatch" style="background:var(--hall)"></span>Hallitus <b>${govTotal}</b></span><span class="item"><span class="swatch" style="background:var(--opp)"></span>Oppositio <b>${oppTotal}</b></span><span class="note">${totalMembers} paikkaa · ${govParties.length + oppParties.length} puoluetta</span></div><table class="party-table">${partyRows}</table>${closeVoteRows}</div>`;
}

function speakersStr(speakers: HomeData["signals"]["speechActivity"]): string {
  ...
  const rows = speakers
    .map((s) => {
      ...
      return `<div class="rail__item"><div class="rail__title"><span style="display:inline-flex;align-items:center;gap:8px;margin-right:4px"><span style="background:${color};width:10px;height:10px;border-radius:50%;display:inline-block;flex:0 0 auto"></span></span>${name}</div><div class="rail__meta">${shortParty} · ${s.speech_count} puheenvuoroa</div></div>`;
    })
    .join("");
  return `<div class="psec"><div class="psec__h"><h2>Eniten puheenvuoroja</h2></div><div class="rail" style="margin-top:12px">${rows}</div></div>`;
}
