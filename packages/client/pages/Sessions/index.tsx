...
/** Inline card for interpellation (VK) documents referenced in sections */
const InterpellationInlineCard: React.FC<{ identifier: string }> = ({
  identifier,
}) => {
  ...
  useEffect(() => {
    let cancelled = false;
    setLoading(true);
    fetch(`/api/interpellations/by-identifier/${encodeURIComponent(identifier)}`)
...
const GovernmentProposalInlineCard: React.FC<{ identifier: string }> = ({
  ...
  useEffect(() => {
    let cancelled = false;
    setLoading(true);
    fetch(`/api/government-proposals/by-identifier/${encodeURIComponent(identifier)}`)
      .then((res) => (res.ok ? res.json() : null))
      .then((json) => {
        if (!cancelled) setData(json);
      })
  ...
  if (loading) {
    return (
      ...
      >
        <CircularProgress size={14} />
        <Typography sx={{ fontSize: "0.75rem", color: colors.textSecondary }}>
          Ladataan hallituksen esityksen tietoja...
        </Typography>
      </Box>
    );
  }
